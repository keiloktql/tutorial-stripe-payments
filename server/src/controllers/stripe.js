...
// Imports
const { createPaymentIntent, updatePaymentIntent, detachPaymentMethod, createSetupIntent, findPaymentMethodFromStripe, updateSubscriptionPaymentMethodInStripe, updateSubscriptionPlanInStripe, updateSubscriptionInStripe, findPaymentIntent } = require('../services/stripe');
...
const { deleteSubscription } = require('../models/subscription');
const { findPlanByPriceID } = require('../models/plan');
const { findInvoice } = require('../models/invoice');
...
module.exports.createSubscription = async (req, res) => {
    try {
        ...
        // 'incomplete', 'active', 'trialing' or 'unpaid'
        const activeSubscription = await findLiveSubscription(accountID);
...
module.exports.updateSubscription = async (req, res) => {
    try {
        ...
        if (plan) {
            ...
            // If subscription is in trial mode, incomplete, or past_due state, allow user to change plan immediately
            // if (activeSubscription.stripe_status === "trialing" || activeSubscription.stripe_status === "incomplete" || activeSubscription.stripe_status === "past_due") {
            // }
        }
    ...
    try {
        ...
        switch (event.type) {
            ...
            case 'invoice.paid': {
                ...
                const paymentMethodID = paymentIntent.payment_intent;
                ...
                if (invoice.billing_reason === 'subscription_create') {
                    ...
                    const account = findAccountByStripeCustID(customerID);
                    const accountID = account.account_id;
                    await updateAccountByID(accountID, {
                        trialed: true
                    });
